<template>
  <div>
    <div class="page-header">
      ...
      <router-link to="/admin/elections/create" class="btn btn-primary">Create New Election</router-link>
    </div>
    ...
    <div v-else-if="elections.length > 0" class="elections-container">
      <div v-for="election in elections" :key="election.id" class="election-card">
        ...
        <div class="election-body">
          <p><strong>Positions:</strong> {{ election.positions_count }}</p>
          <p><strong>Votes:</strong> {{ election.votes_count }}</p>
          <p><strong>Started:</strong> {{ formatDate(election.created_at) }}</p>
        </div>
        <div class="election-actions">
          <router-link :to="`/admin/elections/${election.id}/edit`" class="btn btn-sm btn-secondary">Edit</router-link>
          <button @click="endElection(election.id)" v-if="election.status === 'active'" class="btn btn-sm btn-warning">End</button>
          <button @click="deleteElection(election.id)" class="btn btn-sm btn-danger">Delete</button>
        </div>
      </div>
    </div>
    <div v-else class="empty-state">
      <p>No elections created yet.</p>
      <router-link to="/admin/elections/create" class="btn btn-primary">Create First Election</router-link>
    </div>
  </div>
</template>
