  ...
  methods: {
    async loadElection() {
      try {
        ...
        this.form = {
          ...
          positions: this.election.positions ? this.election.positions.map(position => ({
            name: position.name,
            candidates: position.candidates ? position.candidates.map(candidate => ({
              name: candidate.name
            })) : [{ name: '' }]
    ...
    async updateElection() {
      ...
      try {
        ...
        const response = await adminAPI.updateElection(this.election.id, formData);
        
        alert('Election updated successfully!');
        this.$router.push('/admin/elections');
      } catch (error) {
        console.error('Error updating election:', error);
        alert('Failed to update election: ' + (error.response?.data?.message || error.message));
      } finally {
        this.updating = false;
      }
    }
